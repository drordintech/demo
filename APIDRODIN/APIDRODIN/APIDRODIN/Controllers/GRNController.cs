...
namespace APIDRODIN.Controllers
{
    ...
    public class GRNController : ControllerBase
    {
        ...
        {
            if (challanDto == null) return BadRequest(new { message = "Invalid request: Challan data is missing." });
            if (challanDto.SupplierId == 0) return BadRequest(new { message = "Supplier ID is required." });
            if (challanDto.ChallanDetails == null || !challanDto.ChallanDetails.Any()) return BadRequest(new { message = "At least one Challan detail entry is required." });
                            ...
                            using (SqlDataReader detailReader = await detailCmd.ExecuteReaderAsync())
                            {
                                while (await detailReader.ReadAsync())
                                {
                                    ...
                                    var detailDto = new GRNDetailDto
                                    {
                                        ...
                                        ReturnToParty = detailReader.GetBoolean(17),
        ...
        {
            if (request == null || request.SupplierId <= 0) return Ok(new StockRepairDefaultsResponseDto());
            ...
            try
            {
                ...
                {
                    await connection.OpenAsync();
                    string query = @"SELECT TOP 1 StockReceivedParty, DocketNoDate, Transport, DebitNoteInvoice, DateTime
                                     FROM StockRepair WHERE SupplierId = @SupplierId ORDER BY Id DESC;";
    ...
    {
        public int? SupplierId { get; set; } // 0 or null = All Suppliers
        public int? Year { get; set; }       // e.g., 2025
        public int? Month { get; set; }      // 1 to 12 (leave null for yearly search)
    }
    ...
    public class GRNDto
    {
        ...
        public string DockerNo { get; set; } = null!;
        public string Grnstatus { get; set; } = null!;
        [Required]
        public int SupplierId { get; set; }
        public string? InvoiceReceiptImage { get; set; }
