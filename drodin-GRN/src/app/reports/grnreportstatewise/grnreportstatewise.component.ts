...
@Component({
  selector: 'app-grnreportstatewise',
  standalone: true,
  imports: [FormsModule, CommonModule],
  templateUrl: './grnreportstatewise.component.html',
  styleUrl: './grnreportstatewise.component.scss'
})

export class GrnreportstatewiseComponent implements OnInit, AfterViewInit {
  suppliers: Supplier[] = [];
  dateTo = '';
  dateFrom = '';
  selectedsupplier = '';
  ...
  getStates() {
    this.supplierService.getDistinctStates().subscribe((data) => {
      this.states = data;
    });
  }
  ...
  getReport() {
    ...
    this.grnService.getGrnReportByState(reportParams).subscribe({
      next: (response) => {
  ...
  getPrint(): void {
    ...
    printWindow.document.write(`
      <html>
      <head>
        <title>GRN Report State Wise</title>
      ...
      <body>
        <div class="report-header">
          <h1 class="report-title">GRN Report State Wise</h1>
